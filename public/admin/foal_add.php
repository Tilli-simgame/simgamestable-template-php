...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!validate_csrf_token($_POST['csrf_token'] ?? '')) {
        ...
        if ($f['foal_name'] === '' && $foal_horse_id === null) {
        ...
        if (empty($errors)) {
            ...
            $stmt = $db->prepare(
                'INSERT INTO foals (foal_name, breed_id, sire_id, dam_id, birth_date, gender, status, owner_contact_id, merits, foal_horse_id)
                 VALUES (:foal_name, :breed_id, :sire_id, :dam_id, :birth_date, :gender, :status, :owner_contact_id, :merits, :foal_horse_id)'
            );
            $stmt->execute([
                ':foal_name'        => $f['foal_name'] ?: null,
                ':breed_id'         => $breed_id,
                ':sire_id'          => $sire_id,
                ':dam_id'           => $dam_id,
                ':birth_date'       => $f['birth_date'] ?: null,
                ':gender'           => $f['gender'] ?: null,
                ':status'           => $f['status'] ?: 'born',
                ':owner_contact_id' => $ownerCid,
                ':merits'           => $f['merits'] ?: null,
                ':foal_horse_id'    => $foal_horse_id,
            ]);
            redirect(SITE_URL . '/admin/kasvatus_all.php?added=1');
        }
    }
}

$pageTitle = 'Lisää varsamerkintä';
require __DIR__ . '/includes/admin_header.php';
?>
<div class="admin-page-header">
  <a href="<?= e(SITE_URL) ?>/admin/kasvatus_all.php" class="back-link">← Kasvatus</a>
  <h1>Lisää varsamerkintä</h1>
</div>
...
<form method="post" action="">
  ...
  <div style="display:flex;gap:0.75rem;align-items:center;margin-top:1rem">
    <button type="submit" class="btn">Lisää varsamerkintä</button>
